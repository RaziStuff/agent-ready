# @ahmedshaikh/agent-ready v0.2.10
...
- Detects Composer libraries, Composer plugins, PHPCS standards, Laravel, Symfony, Pest, PHP_CodeSniffer, and Psalm/PHPStan PHP projects, including Composer script aliases and descriptions, Composer build/coverage aliases, WPCS-style aliases, Composer `bin` executable entrypoints and shebang-aware commands, command role/execution metadata, Composer `allow-plugins` guidance, plugin classes, PHPCS standard/ruleset/Sniffs directory roles, compact bundled PHPCS standards grouping, Artisan, Symfony console, Pint, PHPUnit, PHP-CS-Fixer, framework directory roles, and migration risk areas.
...
- `phpcs-standard-package`: Compatibility Standard provides PHP_CodeSniffer sniffs for cross-version PHP compatibility checks.
