...
- `--uses <action-ref>`: set the generated GitHub Action reference. Defaults to `RaziStuff/agent-ready@v0.2.6`.
...
```bash
agent-ready add-to-ci --uses RaziStuff/agent-ready@v0.2.6
agent-ready add-to-ci --write --uses RaziStuff/agent-ready@v0.2.6
```
  ...
  validate:
    ...
    steps:
      ...
      - name: Validate agent metadata
        uses: RaziStuff/agent-ready@v0.2.6
        with:
          command: validate
          mode: required
          strict: "true"
...
- PHP, Composer libraries and plugins, Laravel, Symfony, Pest, Artisan, Symfony console, Pint, PHPUnit, PHP-CS-Fixer, PHPStan, Psalm, plugin classes, and Composer `bin` executable entrypoints.
- Go.
- Rust.
