...
- `--uses <action-ref>`: set the generated GitHub Action reference. Defaults to `RaziStuff/agent-ready@v0.2.10`.
...
```bash
agent-ready add-to-ci --uses RaziStuff/agent-ready@v0.2.10
agent-ready add-to-ci --write --uses RaziStuff/agent-ready@v0.2.10
```
  ...
  validate:
    ...
    steps:
      ...
      - name: Validate agent metadata
        uses: RaziStuff/agent-ready@v0.2.10
        with:
          command: validate
          mode: required
          strict: "true"
...
- PHP, Composer libraries and plugins, PHPCS standards, Laravel, Symfony, Pest, Artisan, Symfony console, Pint, PHPUnit, PHP-CS-Fixer, PHP_CodeSniffer, PHPStan, Psalm, plugin classes, Composer `allow-plugins` guidance, Composer script descriptions, Composer build/coverage aliases, WPCS-style script aliases, PHPCS standard/ruleset/Sniffs directory roles, compact bundled PHPCS standards grouping, Composer `bin` executable entrypoints/commands, and command role/execution metadata.
- Go.
- Rust.
...
- `examples/php-composer-plugin`: Composer plugin with Psalm, plugin class resolution, and Composer `bin` executables.
- `examples/php-code-quality-package`: Composer package with PHP_CodeSniffer, `allow-plugins`, PHPCS rulesets, and CLI bin commands.
- `examples/phpcs-standard-package`: PHPCS standard package with Composer `bin` shebang handling and long-running command metadata.
