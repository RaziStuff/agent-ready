...
- `--uses <action-ref>`: set the generated GitHub Action reference. Defaults to `RaziStuff/agent-ready@v0.2.1`.
...
```bash
agent-ready add-to-ci --uses RaziStuff/agent-ready@v0.2.1
  ...
  validate:
    ...
    steps:
      ...
      - name: Validate agent metadata
        uses: RaziStuff/agent-ready@v0.2.1
        with:
          command: validate
          mode: required
          strict: "true"
...
- PHP, Composer, Laravel, Symfony, Artisan, Symfony console, Pint, PHPUnit, PHP-CS-Fixer, and PHPStan hints.
- Go.
- Rust.
- Ruby on Rails.
- Django.
